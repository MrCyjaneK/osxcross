...
do
  ...
  # Using full fledged Xcode
    ...
    if [ $(uname -s) != "Darwin" ]; then
      ...
      if [ -d "Contents/Developer/Platforms/$platform/Developer/SDKs" ]; then
        pushd "Contents/Developer/Platforms/$platform/Developer/SDKs" &>/dev/null
      else
        if [ -d "../Packages" ]; then
    ...
    for pat in $pplatform[0-9]*.sdk $pplatformX[0-9]*.sdk; do
      while IFS= read -r d; do
        if [[ "$d" != "$pplatform.sdk" && "$d" != "$pplatformX.sdk" && "$d" != *Patch* && -d "$d" ]]; then
          SDKS+=("$d")
        fi
      done < <(compgen -G "$pat")
    done
